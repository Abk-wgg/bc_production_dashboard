...
import { isBoardLocation } from "../scope";

/**
 * Every production order header the web service returns.
 *
 * Only orders at the PRODUCTION location - see src/lib/scope.ts. TRADE orders
 * are excluded here rather than in the UI, so the tables, the schedule and the
 * JSON feeds cannot end up disagreeing about what is on the board.
 *
...
export async function getProductionOrders(): Promise<Fetched<ProductionOrder>> {
  ...
  const rows = result.rows
    ...
    .filter((order) => isBoardLocation(order.locationCode));
  return { ...result, rows };
}
